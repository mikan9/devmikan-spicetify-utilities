...
import {
	fetchAndStoreFollowing,
	fetchAndStorePresenceViews,
	getStoredPresenceViewsByUserId,
	maxViews,
	PresenceViewHandler,
	removeDuplicates,
	storePresenceViewForUser,
	users,
...
> {
	...
	subscribeToBuddyActivity() {
		const sub = Spicetify.Platform.BuddyFeedAPI.subscribeToBuddyActivity(
			users.l,
			(e: IPresenceView) => {
				console.log("BuddyActivity: ", e);

				const views = storePresenceViewForUser(users.l, e);
				this.updateViews(views);
			}
		);
	}

	updateViews(views: IPresenceView[]) {
		views = views.sort((a, b) => b.timestamp - a.timestamp);

		if (views.length > maxViews) views = views.slice(0, maxViews);

		if (
			views.length > this.viewHandler.currentViews.length ||
			views[0].timestamp !== this.state.views[0].timestamp
		) {
			this.viewHandler.currentViews = views;
			this.setState((state) => {
				return {
	...
	fetchViews() {
		fetchAndStorePresenceViews().then((views) => {
			this.updateViews(views);
		});
	}
	...
	componentDidMount(): void {
		...
		(() => {
			...
			window.setInterval(fn, 1000 * 60 * 15);
			fn();
		})();
	}
	...
	exportToJSON = () => {
		...
		ele.value = JSON.stringify(getStoredPresenceViewsByUserId(users.l)); //JSON.stringify(this.viewHandler.currentViews);
					...
					<div>
						<div className={styles.pvHeader}>
							...
							<div className={styles.buttonBar}>
								<button
									className="Button-buttonSecondary-small-useBrowserDefaultFocusStyle encore-text-body-small-bold"
